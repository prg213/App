...
Generated unified diffs for patched native dependencies can be syntactically valid when tested from a temporary file but become corrupt after an editor-style workspace write, especially when the final hunk ends with a blank context line. Every unchanged line in the patch must retain its leading unified-diff context space. Validate the committed patch with a clean pnpm install and inspect the resulting installed native source, not only a cached package copy.
