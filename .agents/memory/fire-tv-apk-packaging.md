...
**How to apply:** Restrict React Native and Android NDK packaging to physical ARM targets, then build non-universal ABI splits. Stage the ARM32 file under the stable `StreamVault.apk` name and publish both named ARM32 and ARM64 assets. Keep device-aware updater selection and legacy fallbacks so existing installs transition safely. Set Android `versionCode` from the monotonically increasing CI build number so package updates are accepted.