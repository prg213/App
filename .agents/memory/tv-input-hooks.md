...
- `useBackHandler(handler, enabled?)` — hardware Back; focus-aware (useFocusEffect internally), handler kept in a ref so no memoization needed. `enabled` gates conditional handlers (modal visible, live-only) and re-registration moves the listener to the front of the LIFO queue.
...
**Why:** BackHandler was once registered 9 times across screens with subtle differences; a plain `useEffect` registration on Live TV competed with other screens' handlers even when not focused (active correctness bug). Media keys were siloed in one screen.
