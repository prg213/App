...
- **tvRowNav** (`lib/tvRowNav.ts`): the main content is a fixed 4-column × 3-row grid (Continue Watching, Latest Movies, Latest TV Shows); Recently Watched stays separate. Cards register nodes by (rowId,index); UP/DOWN preserves the current column and clamps to the nearest available card in a shorter row, pinned at edges.
